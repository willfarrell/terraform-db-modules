...
Host proxy
  ...
  LocalForward ${DB_FROM_PORT} ${DB_HOST}:${DB_TO_PORT}
...
if [ "${SSH_USERNAME}" != "" ]; then
	#ssh -4fNT -o StrictHostKeyChecking=no -i /root/.ssh/$_{SSH_IDENTITY_FILE} $_{SSH_USERNAME}@$_{BASTION_IP} -L $_{DB_FROM_PORT}:$_{DB_HOST}:$_{DB_TO_PORT}
	ssh -4fNT proxy
	DB_HOST="127.0.0.1"
fi

cat /data/${INIT_SCRIPTS_FOLDER}/*.sql | psql -h $DB_HOST -p ${DB_FROM_PORT} -d ${DATABASE_NAME} -1 -o /data/postgres_sql.txt
